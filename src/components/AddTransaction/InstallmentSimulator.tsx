...
export function InstallmentSimulator({ amount, installments }: InstallmentSimulatorProps) {
  ...
  return (
    <div className="p-3 bg-gray-50 rounded-lg border border-[#DEDEDE]">
      <h4 className="text-sm font-medium text-[#121212] mb-2">Simulação de Parcelas</h4>
      <div className="space-y-1 max-h-32 overflow-y-auto">
        {Array.from({ length: Math.min(installments + 2, 6) }, (_, i) => {
          ...
          return (
            <div 
              key={currentInstallments} 
              className={`flex justify-between text-sm ${
                isSelected ? 'font-semibold text-[#61710C]' : 'text-[#64748B]'
              }`}
            >
